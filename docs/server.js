...
// taken from https://github.com/AlexxNB/svelte-docs/blob/master/packages/server/server.js
// modified to enable CORS
...
const DIR = DEV
  ? path.join(CWD, config.pathes.dev)
  : path.join(CWD, config.pathes.build)
...
createServer(mw).listen(port, hostname, (err) => {
  ...
  const srv = DEV
    ? chalk.yellow('DEVELOPMENT server started...')
    : chalk.green('Server started...')

  clear(true)
  console.log(`
    ${chalk.bold('Svelte-Docs:')} ${srv}
    Please open: ${chalk.blue(
      'http://' +
