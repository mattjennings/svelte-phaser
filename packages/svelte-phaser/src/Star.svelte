...
<script lang="ts">
  ...
  export let instance = new GameObjects.Star(
    scene,
    x,
    y,
    points,
  ...
  $: shouldApplyProps(innerRadius) && instance.setInnerRadius(innerRadius)
  $: shouldApplyProps(outerRadius) && instance.setOuterRadius(outerRadius)
