...
<script lang="ts">
   ...
   */
  export let fillStyle: Phaser.Types.GameObjects.Graphics.FillStyle = undefined
  ...
  export let instance: Phaser.GameObjects.Graphics = new Phaser.GameObjects.Graphics(
    scene,
    {
      fillStyle,
      lineStyle,
  ...
  $: if (shouldApplyProps(fillStyle)) {
    instance.fillStyle(fillStyle.color, fillStyle.alpha)
  }

  $: if (shouldApplyProps(lineStyle)) {
    instance.lineStyle(lineStyle.width, lineStyle.color, lineStyle.alpha)
  }
